"""Five hundred great-circle arcs, their Clairaut boxes checked against dense sampling.

The drill draws five hundred random arcs between latitudes -70 and
70, computes each arc's bounding box from Clairaut's constant, and
compares it with the box of four thousand points sampled along the
...
step on every arc, and the endpoint box missed the arc by more than
a degree of latitude on about two in five of them, by tens of
degrees on the worst, so a spatial index filed on endpoint boxes
would lose a large share of long routes. The survey keeps the
endpoints-will-do guess beside the measured miss rate.
"""
...
def run() -> Survey:
    ...
    holds = worst_gap < 1e-3 and missed > 100
    return Survey(
        surveyor="arcwatch",
        finding=(
            "Clairaut boxes matched dense sampling within the sampling step on all "
            "500 arcs while endpoint boxes missed about two in five by over a "
            "degree, 71 degrees at worst"
        ),
        readings=readings,
        holds=holds,
    )
